...
Last Sync: 2026-05-12T14:36:39.222743
...
- **`CallWorkflowStep`**: Per-call configuration of the evaluation steps — which ones run, who acts on them, their durations, gates and transition behaviour
- **`Round`**: Time-bounded submission window; carries scheduling only
...
## Round Scheduling and Workflow Configuration

Scheduling and evaluation policy are configured in two different places, and
the split matters when reading older code or documentation:

- **`Round`** — *when* proposals may be submitted, and when an award is dated.
- **`CallWorkflowStep`** — *how* a submitted proposal is evaluated, one row per
  step per call.

`Round.review_strategy`, `Round.deciding_entity`, `Round.minimal_average_scoring`
and `Round.minimum_number_of_reviewers` were removed in migration
`0061_collapse_round_config_to_scheduling`, and `Round.allocation_time` in
`0062_move_allocation_time_to_workflow_step`. Configuration that used to live on
those fields is now expressed as workflow steps.

### Round configuration

Beyond its call and slug, a round carries only scheduling:

| Field | Meaning |
|-------|---------|
| **`start_time`** | When submissions open |
| **`cutoff_time`** | When submissions close |
| **`review_duration_in_days`** | Per-review deadline; drives `Review.review_end_date`. Defaults from the `PROPOSAL_REVIEW_DURATION` Constance setting |
| **`allocation_date`** | Optional date an award takes effect, used only by steps configured with `allocation_time = fixed_date` |

`Round.status` is derived from the clock, not stored:

| `status` | Condition |
|----------|-----------|
| **`scheduled`** | `start_time` is in the future |
| **`open`** | `start_time` has passed and `cutoff_time` has not |
| **`ended`** | `cutoff_time` has passed |

Rounds on the same call may not overlap, and `cutoff_time` must be later than
`start_time`. There is no maximum round length.

### The workflow step catalog

Steps are a fixed catalog (`enums.WORKFLOW_STEPS`) and always execute in catalog
order. A call enables the ones it wants by creating a `CallWorkflowStep` row;
a step with no row — or with `is_enabled=False` — is skipped.

| Step | Default responsible role | Notes |
|------|--------------------------|-------|
| **`administrative_check`** | Call manager | Eligibility and completeness check |
| **`technical_assessment`** | Offering manager | Service provider confirms feasibility |
| **`expert_review`** | Reviewer | Independent scored peer review |
| **`panel_review`** | Panel member | Requires `expert_review` to be enabled |
| **`allocation_decision`** | Call manager | **Mandatory**; approving it accepts and provisions the proposal |
| **`award_response`** | Applicant | Applicant accepts or declines the award |

Per-step configuration lives on `CallWorkflowStep`:

| Field | Effect |
|-------|--------|
| **`duration_in_days`** | Deadline for the step, set when it activates. `mark_expired_workflow_steps` expires it and advances the workflow |
| **`checklist`** / **`checklist_required`** | Evaluation form for the step, and whether it blocks completion |
| **`min_reviewers`** / **`min_score_threshold`** | Completion *gates* on `expert_review` — a human still decides; nothing auto-accepts above a score |
| **`blind_review`**, **`requires_coi_confirmation`** | Evaluator-facing constraints |
| **`transition_mode`** | `automatic_on_completion` advances immediately; `manual` parks the proposal until a separate advance action |
| **`allocation_time`** | On `allocation_decision`: `on_decision` starts the allocation immediately, `fixed_date` dates it to the round's `allocation_date` |
| **`include_award_response`** | On `allocation_decision`: require an explicit applicant response before provisioning |
| **`applicant_visible`** | Whether the applicant sees step detail rather than just its status |

A call can only be activated once it has a round, at least one enabled workflow
step including the mandatory `allocation_decision`, and an accepted offering
with a plan.

### Evaluation starts at submission

There is no "review after the round closes" mode. `ProposalViewSet.submit`
creates a `ProposalWorkflowStepInstance` for every catalog step in one
transaction — `pending` for enabled steps, `skipped` for the rest — activates
the first enabled one, and moves the proposal to `in_review` immediately. Only
a call with no enabled steps leaves a proposal in `submitted`.
...
sequenceDiagram
    ...
    A->>P: submit
    P->>P: create step instances (pending / skipped)
    P->>W: activate first enabled step, state = in_review
    loop each enabled step
        W->>W: responsible role completes the step
        alt transition_mode = manual
            W->>W: wait for explicit advance
        end
    end
    W->>P: no step left → state = accepted
    W->>M: allocate_proposal — project, roles, orders
```

Reaching the end of the enabled steps both accepts the proposal and provisions
it: `allocate_proposal` creates the project, applies the
`ProposalProjectRoleMapping` entries and places the marketplace orders. It is
guarded on the proposal not already having a project, so it provisions exactly
once.

### Closing a round is a hard stop

Closing a round cancels its **draft** proposals (`process_closed_round`).
Separately, the hourly `proposals_for_ended_rounds_should_be_cancelled` task
cancels every proposal on a round whose `cutoff_time` has passed that has not
reached `accepted`, `rejected` or `canceled` — including proposals still under
evaluation. A cutoff is therefore a deadline for the whole evaluation, not just
for submission: size the round so reviews can finish inside it.

### Continuously open (rolling) calls

There is no "always open" flag; a rolling call is a shape, not a mode:

1. Create **one round** with `start_time` set to now and `cutoff_time` far in
   the future. `status` is derived, so the round reads `open` indefinitely, and
   no proposal is swept up by the cutoff task while it is being evaluated.
2. Enable only the steps that can run without a reviewer pool — typically
   `administrative_check` and the mandatory `allocation_decision`. Reviews are
   never created automatically (see [Review Assignment](#review-assignment)),
   so an enabled `expert_review` step parks each proposal until the call
   manager builds an assignment batch.
3. Leave `allocation_time = on_decision` on `allocation_decision` and leave the
   round's `allocation_date` unset, so an approval takes effect at once instead
   of waiting for a shared date.
4. Set `review_duration_in_days` to the turnaround you want per review, not to
   the length of the round.
5. Set `Call.fixed_duration_in_days` so every award runs the same length from
   its own provisioning rather than to a shared end date.

The cost of the workaround is cosmetic: applicants see the far-future cutoff
presented as a submission deadline throughout the UI.
...
Reviews are never created automatically by submission or by a round closing.
Assignment is a two-stage, manager-driven workflow, and a `Review` row exists
only once a reviewer has accepted an invitation:

```mermaid
sequenceDiagram
    participant M as Call manager
    participant Pool as CallReviewerPool
    participant B as AssignmentBatch
    participant Rev as Reviewer
    participant R as Review

    M->>Pool: Build reviewer pool (invitations, bids, profiles)
    M->>B: Generate batch per reviewer (algorithm or manual)
    Note over B: status = draft
    M->>B: Send batch
    Note over B: status = sent, expires_at set
    B->>Rev: Notify — one AssignmentItem per proposal
    alt Reviewer accepts an item
        Rev->>R: Review created in IN_REVIEW
    else Reviewer declines
        Rev->>B: Item declined; optional auto-reassign
    else No response before expiry
        B->>B: mark_expired_assignment_batches → expired
    end
    Rev->>R: Submit review
```

An `AssignmentBatch` targets one reviewer pool entry and holds one
`AssignmentItem` per proposal. `CallAssignmentConfiguration` controls the
invitation lifetime (`assignment_expiration_days`), reminder timing and whether
declines are automatically reassigned to the next-best reviewer. A review left
open past `review_end_date` — derived from the round's `review_duration_in_days`
— is rejected by the hourly `expired_reviews_should_be_cancelled` task.
...
# Scheduling: a six-week submission window, awards dated to 1 March
round = Round.objects.create(
    call=call,
    start_time=datetime(2024, 1, 1),
    cutoff_time=datetime(2024, 2, 15),
    review_duration_in_days=21,
    allocation_date=datetime(2024, 3, 1)
)

# Evaluation policy: blind expert review gated on three reviews averaging
# at least 7.0, then a call manager decision dated to the allocation date.
CallWorkflowStep.objects.create(
    call=call,
    step="expert_review",
    blind_review=True,
    min_reviewers=3,
...
2. The call manager assigns reviewers; expert review starts as each batch is accepted
3. The `expert_review` step cannot be completed until three reviews average ≥7.0
4. The call manager approves the `allocation_decision` step, which accepts and provisions the proposal
5. HPC accounts are created with the specified limits and dated to 1 March
6. Usage tracked through marketplace billing

The score threshold is a gate on completing the step, not an auto-accept: no
proposal is granted without a human approving the allocation decision.
...
# Quarterly rounds; one round per quarter, non-overlapping
...
# Expert review feeding a panel, then a manually confirmed decision.
# panel_review requires expert_review to be enabled.
CallWorkflowStep.objects.create(call=call, step="expert_review", min_reviewers=3)
CallWorkflowStep.objects.create(call=call, step="panel_review")
CallWorkflowStep.objects.create(
    call=call,
    step="allocation_decision",
...
2. Expert reviews are collected, then consolidated by the panel step
3. Program managers approve the allocation decision for the proposals they select
4. Resources are dated to the round's `allocation_date` — 1 April for Q1
5. Multi-year projects supported with renewal process

Each quarter needs its own round: rounds on one call may not overlap, and a
proposal still under evaluation when its round's cutoff passes is cancelled by
the hourly sweep. Schedule the cutoff to leave room for the whole workflow, or
close the round explicitly once decisions are made.
...
# One long-lived round stands in for a continuously open call. The cutoff
# must outlast every evaluation started under it: proposals still open when
# it passes are cancelled by proposals_for_ended_rounds_should_be_cancelled.
rolling_round = Round.objects.create(
    call=call,
    start_time=datetime(2024, 1, 1),
    cutoff_time=datetime(2030, 12, 31),
    review_duration_in_days=14  # Fast turnaround, per review
)

# Only steps that need no reviewer pool, so a decision can be taken as soon
# as the application lands. Evaluation begins at submission — there is no
# setting to switch on for that.
CallWorkflowStep.objects.create(
    call=call,
...
CallWorkflowStep.objects.create(
    call=call,
    step="allocation_decision",
    allocation_time=AllocationTimes.ON_DECISION  # default; awards start at once
)
...
1. Startups apply continuously; each submission activates the administrative
   check immediately and moves the proposal to `in_review`
2. Incubator staff clear the check, then approve or reject the allocation
   decision — no waiting for a cohort to close
3. Resources are provisioned the moment the decision step completes, because
   `allocation_time` is `on_decision` and the round has no `allocation_date`
4. `Call.fixed_duration_in_days` gives every award the same length from its own
   provisioning date
5. Renewal is a fresh proposal on the same round

Enabling `expert_review` here would break the "decide at once" property: a
review exists only after a call manager builds an assignment batch and a
reviewer accepts it, so the proposal would park on that step in the meantime.
