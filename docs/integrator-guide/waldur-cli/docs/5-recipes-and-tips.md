...
Last Sync: 2026-09-15T18:07:15.015219
...
!!! note "Instances need a few things a VPC doesn't"
    ...
    below). For the subnet, either provision the tenant **without** the
    `skip_creation_of_default_*` attributes to get a default network + subnet, or build the
    network yourself (see [Build a tenant's network by hand](#build-a-tenants-network-by-hand)).
...
### Build a tenant's network by hand

Networks, security groups, and floating IPs have no REST `create` of their own: Waldur creates
them through actions on the tenant, and a subnet through an action on its network. Each is
created asynchronously (`Creating` → `OK`), and a network must be `OK` before it takes a
subnet, so `wait` between the steps:

```bash
TENANT=<tenant-uuid>

# 1. a network, and a subnet in it (attached to the tenant's router unless the request
#    sets "skip_router_connection": true)
NET=$(waldur-cli openstack tenant create-network "$TENANT" \
  --request '{"name": "app-net"}' --format json | jq -r .uuid)
waldur-cli openstack network wait "$NET" --jmespath "state=='OK'"
waldur-cli openstack network create-subnet "$NET" \
  --request '{"name": "app-subnet", "cidr": "10.10.0.0/24", "dns_nameservers": ["1.1.1.1"]}'

# 2. a security group that lets SSH in
waldur-cli openstack tenant create-security-group "$TENANT" --request '{
  "name": "ssh",
  "rules": [{"ethertype": "IPv4", "direction": "ingress", "protocol": "tcp",
             "from_port": 22, "to_port": 22, "cidr": "0.0.0.0/0"}]
}'
```

Every action that takes a body has a `--generate-skeleton` template, printed without a UUID:
`waldur-cli openstack network create-subnet --generate-skeleton`.

To change an existing group's rules, `security-group set-rules <uuid>` takes a JSON *array*
of rules and **replaces** the whole rule set, so include every rule you want to keep. Routers
(`openstack router create`, `add-router-interface`, `set-external-gateway`, `set-routes`) and
ports (`openstack port disable-port-security`, `set-allowed-address-pairs`) cover what the
tenant's default router and an instance's default port don't, such as a VM that routes
