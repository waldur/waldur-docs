...
### The provider's requirement always applies

The offering's **Require purchase order upload** setting governs order approval on its own, whatever the call collected. The two can disagree: a call takes its copy of the setting when the offering is added, so an offering that starts requiring a purchase order later leaves earlier calls collecting nothing.

Where they disagree, the order is not approved automatically. It waits in **pending consumer approval** until somebody uploads the document, exactly as an ordinary order from the marketplace would.

!!! note
    Only the document satisfies the provider's requirement at this point. A reference on its own is enough to submit the proposal, but an order for an offering that requires the upload still waits for the file.

To avoid this, turn the requirement on before adding the offering to a call, or re-add the offering afterwards so the call picks up the current setting.
...
| An allocated order waits for consumer approval although the proposal carried a purchase order | The offering requires the document and only a reference was given, or the offering began requiring one after it was added to the call. Upload the document on the order to approve it. |
