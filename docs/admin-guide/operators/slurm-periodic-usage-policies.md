...
flowchart TD
    ...
    Sub -- No --> FixSub["No event_process agent, or<br/>stomp/periodic_limits disabled —<br/>see Layer 3"]
...
- Confirm the agent is connected and that a `resource_periodic_limits`
  ...
  ```
    ...
    !!! warning "Only the `event_process` agent registers this queue"
        The `order_process`, `membership_sync` and `report` agents never create
        event subscriptions, so restarting them changes nothing here — a
        deployment running only those modes ("polling-based processing") cannot
        use periodic policies at all. Check that an agent runs in
        `event_process` mode for this offering (Helm:
        `agents.eventProcess.enabled`), that the offering has
        `stomp_enabled: true` — it defaults to `false` — and that
        `backend_settings.periodic_limits.enabled` is `true`. Subscriptions are
        computed once at startup, so restart the **`event_process`** agent after
        any of those changes. Requires site agent 0.8.0 or newer.

  In the `event_process` agent log, successful registration looks like:
  ...
  If the first line is missing, the agent never saw the setting (wrong mode,
  wrong config file, or too old a version) — the negative branch logs at DEBUG
  only. If the second line is followed by `Failed to create event subscription
  queue`, the API rejected the registration; the error text names the reason.
