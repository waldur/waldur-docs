...
Branch: develop
Remote Path: docs/admin/scheduled.md
Local Path: docs/admin-guide/mastermind-configuration
Last Sync: 2026-05-04T02:03:59.407154

WARNING: This file is automatically synchronized from the source repository.
DO NOT EDIT this file directly. Changes will be overwritten.
Edit the source at: https://code.opennodecloud.com/waldur/waldur-mastermind.git/-/tree/develop/docs/admin/scheduled.md
-->
...
| `cleanup_usage_poll_records` | `waldur_mastermind.marketplace.cleanup_usage_poll_records` | 1 day | Delete ComponentUsagePollRecord entries older than the retention period. |
...
| `openstack-tenant-usage-billing-poll` | `openstack.TenantUsageBillingPoll` | 30 minutes | Poll quota usage for tenants whose offering uses USAGE billing.<br><br>Runs on a 30-min base tick. Per-offering throttle (via Django cache)<br>ensures each offering is polled at its configured interval.<br>Dispatches a separate Celery task per tenant to leverage multiple workers. |
...
| `reconcile-user-roles-against-availability` | `waldur_core.permissions.reconcile_user_roles_against_availability` | 1 day | Periodic safety net: reconcile UserRoles against RoleAvailability for<br>every role that has at least one availability row.<br><br>Catches drift from manual DB edits / failed signals. |
...
| `terminate_resources_if_project_end_date_has_been_reached` | `waldur_mastermind.marketplace.terminate_resources_if_project_end_date_has_been_reached` | Cron: `40 1 * * * (m/h/dM/MY/d)` | Terminate resources when their project has reached its end date (including grace period).<br><br>Also pauses resources for projects currently in the grace period,<br>if the offering has supports_pausing=True in plugin_options. |
