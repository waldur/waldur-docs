...
class LocalDocsLister:
    ...
    def scan_directory(self, docs_path, include_patterns=None, exclude_patterns=None):
        ...
        results = {"local": [], "external": [], "preserved": []}
        ...
        for pattern in include_patterns:
            for file_path in docs_path.rglob(pattern):
                ...
                if any(
                    fnmatch.fnmatch(file_path.name, exp) for exp in exclude_patterns
                ):
                    continue
                ...
                under_external, external_path = self.is_under_external_path(
                    file_path, external_paths
                )
    ...
    def print_results(self, results, output_format="summary", show_paths=True):
        ...
        if output_format == "summary":
            ...
            if show_paths:
                ...
                print(
                    f"  {len(results['external'])} files imported from external sources"
                )
            ...
            if not results["preserved"]:
                ...
                for item in sorted(results["preserved"], key=lambda x: x["relative"]):
                    ...
                    if item["external_path"]:
                        print(
                            f"    (Preserved in external path: {item['external_path']})"
                        )
...
def main():
    ...
    parser.add_argument(
        "--docs-path", "-d", default="docs", help="Documentation directory path"
    )
    parser.add_argument(
        "--format",
        "-f",
        choices=["summary", "detailed", "paths", "stats"],
        default="summary",
        help="Output format",
    )
    parser.add_argument(
    ...
    try:
        ...
        results = lister.scan_directory(
            args.docs_path, include_patterns=args.include, exclude_patterns=args.exclude
        )

        lister.print_results(
            results, output_format=args.format, show_paths=not args.no_paths
        )
