...
class ExternalSyncValidator:
    ...
    def check_file_sync_status(self, file_path):
        ...
        if last_sync:
            try:
                ...
                age_hours = (
                    datetime.now() - sync_time.replace(tzinfo=None)
                ).total_seconds() / 3600
                ...
                return {
                    ...
                    "message": "Invalid sync timestamp format",
                }
        ...
        for other_source_key, other_mappings in all_mappings.items():
            for other_mapping in other_mappings:
                ...
                if (
                    ...
                    and other_mapping.get("local") == mapping.get("local")
                    and other_mapping.get("remote") == mapping.get("remote")
                ):
                    continue
    ...
    def validate_mapping(self, source_key, source, mapping, all_mappings):
                ...
                "issues": [
                    {
                        ...
                        "message": f"Local directory {mapping['local']} does not exist",
                    }
                ],
            }
        ...
        for file_path in local_path.rglob("*.md"):
            ...
            if self.should_exclude_from_source(
                file_path, source_key, mapping, all_mappings
            ):
                continue
        ...
        for mapping in source.get("mappings", []):
            ...
            if result["issues"]:
                ...
                for issue in result["issues"]:
                    print(
                        f"      - {issue['type']}: {issue.get('file', '')} - {issue['message']}"
                    )
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate external documentation sync status"
    )
    parser.add_argument(
        "--config", "-c", default="external-sources.yml", help="Configuration file path"
    )
    parser.add_argument(
        "--sources", "-s", nargs="*", help="Specific sources to validate (default: all)"
    )
    parser.add_argument("--report", "-r", help="Generate detailed report to file")
