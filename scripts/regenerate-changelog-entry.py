...
def main():
    parser = argparse.ArgumentParser(
        description="Regenerate changelog entries with enhanced format"
    )
    parser.add_argument(
        "version", nargs="?", help="Version to regenerate (e.g., 7.9.0)"
    )
    parser.add_argument(
        "prev_version", nargs="?", help="Previous version (e.g., 7.8.9)"
    )
    parser.add_argument(
        "--update-recent", type=int, metavar="N", help="Update N most recent entries"
    )
    parser.add_argument(
        "--list-versions",
        action="store_true",
    ...
    if args.update_recent:
        ...
        print(
            "  python3 regenerate-changelog-entry.py 7.9.0  # auto-detect previous version"
        )
